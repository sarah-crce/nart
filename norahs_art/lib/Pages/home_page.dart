...
import 'package:norahs_art/Pages/settings_page.dart';
import 'package:norahs_art/bloc/image_cubit.dart';
import 'package:path/path.dart' as xp;
import 'package:norahs_art/utils/app_language.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _saveImage(Uint8List canvas) async {
    ...
    try {
      if (path != pathHint) {
        ...
        final image =
            '''IMG-${DateTime.now().day.toString()}-${DateTime.now().month.toString()}-${DateTime.now().year.toString()}--${DateTime.now().hour.toString()}-${DateTime.now().minute.toString()}-${DateTime.now().millisecond.toString()}-NArt.png''';
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Scaffold(
        backgroundColor: Color(0xFFC197D2),
        key: scaffoldKey,
        appBar: AppBar(
          actions: [
            InkWell(
